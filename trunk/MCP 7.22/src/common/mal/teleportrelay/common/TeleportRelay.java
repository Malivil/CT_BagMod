...
public class TeleportRelay {
	...
	public void oPreInit(final FMLPreInitializationEvent event) {
		...
		try {
			cfg.load();
			teleportRelayID = cfg.getBlock("teleportRelayID", 150).getInt(150);
			relayRemoteID = cfg.getItem("relayRemoteID", 4500).getInt(4500);
			relayRemoteUses = cfg.getItem("relayRemoteUses", 10).getInt(10);
			allowRepair = cfg.get("allowRepair", Configuration.CATEGORY_GENERAL, false).getBoolean(false);
			requirePower = cfg.get("requirePower", Configuration.CATEGORY_GENERAL, false).getBoolean(false);
			debug = cfg.get("debug", Configuration.CATEGORY_GENERAL, false).getBoolean(false);
		}
		catch (Exception e) {
	...
	public void onInit(final FMLInitializationEvent event) {
		...
		GameRegistry.addRecipe(new ItemStack(relayRemoteItem, 1), new Object[] { "b", "i", "r", Character.valueOf('b'), Block.stoneButton, Character.valueOf('i'), Item.ingotIron, Character.valueOf('r'), Item.redstone });
	...
	public static void printRelayLocations() {
		...
		final ChunkCoordinates bed = minecraft.thePlayer.getBedLocation();
